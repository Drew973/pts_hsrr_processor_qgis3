...
RETURNS table (pk int) AS $$
	...
	,f as (
	select run,start_run_ch,lead(start_run_ch) over (order by start_run_ch),end_run_ch,sec,start_sec_ch,end_sec_ch from a
		where (select count(sec) from a as b where b.score<a.score and a.r&&b.r) = 0
		and (select count(sec) from e where a.r&&e.r) = 0
		)
		
	insert into hsrr.section_changes(run,ch,sec,start_sec_ch,end_sec_ch)
	select run,start_run_ch,sec,start_sec_ch,end_sec_ch from f
	union
	select run,end_run_ch,'D',0,0 from f where end_run_ch<lead--dummys where gaps.
