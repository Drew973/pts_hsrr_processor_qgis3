...
RETURNS void AS $$	
    BEGIN
		...
		insert into section_changes(run,ch,geom,note)
		select run,e_ch,e_point,'auto_end' from a where lead is null or not st_dwithin(vect,lead,10);
	
	
		--starts
		with a as (select run,s_ch,s_point,vect,lag(vect) over (partition by run order by s_ch) from readings where run=rn)
		insert into section_changes(run,ch,geom,note)
		select run,s_ch,s_point,'auto_start' from a where lag is null or not st_dwithin(vect,lag,10);	

		--nodes
		insert into section_changes(run,ch,geom,note)
		select run,s_ch+0.1*st_lineLocatePoint(vect,p) as ch,st_closestPoint(vect,p) as p,'auto_node'
