...
'SELECT cast(meas_len as float) from network where sec=sect' LANGUAGE sql IMMUTABLE;

CREATE OR REPLACE FUNCTION road(sect varchar) RETURNS varchar AS
'SELECT road from network where sec=sect' LANGUAGE sql IMMUTABLE;

CREATE OR REPLACE FUNCTION is_rbt(sect varchar) RETURNS bool AS
'SELECT rbt from network where sec=sect' LANGUAGE sql IMMUTABLE;

CREATE OR REPLACE FUNCTION near_rbt(pt geometry('point'),dist float=5) RETURNS bool AS
'SELECT 0<count(sec) from network where rbt and st_dwithin(pt,geom,dist)' LANGUAGE sql IMMUTABLE;

CREATE OR REPLACE FUNCTION invert_ch(ch float,sect varchar) RETURNS float AS
'SELECT meas_len-ch from network where sec=sect' LANGUAGE sql IMMUTABLE;
					   

CREATE OR REPLACE FUNCTION ch_to_point(sect varchar,chainage float) 
RETURNS geometry('point') AS $$
		declare 	
			L float=meas_len from network where sec=sect;
			f float=chainage/L;			
			geom geometry('linestring')=geom from network where sec=sect;
        BEGIN	
			if f>1  then 
				f=1;
			end if;
...
RETURNS int AS $$
	declare f int=10*floor(meas_len/10) from network where sec=sect;
        BEGIN	
			if f<meas_len from network where sec=sect then
				return f;
			else 
				return f-10;
			end if;
...
RETURNS float AS $$
        BEGIN	
			return meas_len-ch from network where sec=sect;
		END;			
$$ LANGUAGE plpgsql;
...
RETURNS int AS $$
        BEGIN	
			return meas_len from network where sec=sect;
		END;			
$$ LANGUAGE plpgsql;
...
RETURNS float AS $$
        BEGIN	
			return calc*(select meas_len from network where sec=sect)/(select calc_len from network where sec=sect);
		END;			
$$ LANGUAGE plpgsql;
